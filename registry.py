...
class ClientThread(threading.Thread):
    ...
    def Search(self, message):
        ...
        # enters if username does not exist
            ...
            logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
            self.tcpClientSocket.send(response.encode())
    # this function is used to get the online users from the database and send iit to the peer
    def ListOnlineUsers(self, message):
        ...
        while True:
            try:
                ...
                if len(message) == 0:
                    break
                #   JOIN    #
                if message[0] == "JOIN" or message[0] =="LOGIN":
